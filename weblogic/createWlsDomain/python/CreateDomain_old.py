...
def parseCluster():
    ...
    for name in keys.split(","):
		...
		createCluster(name, clusterAddress)

def createCluster(name, clusterAddress):	
...
def createCnxFactory(name,jndiName,module):
	...
	try:
		cd('/JMSSystemResources/'+ module + '/JMSResource/'+ module + '/ConnectionFactories/')
		cmo.createConnectionFactory(name)
	except Exception:
		dumpStack()
		print 'Erreur lors de la creation de la connection Factory ' + name
...
def parseJMSDestination():
	...
	for name in keys.split(","):
	  ...
	  module = token[2]
	  subs = token[3]
	  createJMSDestination(name,jndiName,type,module,subs)

def createJMSDestination(name,jndiName,type,module,subs):
	...
	try:
		...
		print 'Erreur lors de la creation de la ressource JMS ' + name
